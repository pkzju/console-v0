...
const unsigned short CRC_INIT = 0x1862;
/*
...
**  Output:       CRC checksum
*/
unsigned short Get_CRC16_Check_Sum(U8 *pchMessage,U32 dwLength,unsigned short wCRC)
{
    U8 chData;
    if (pchMessage == NULL)
    {
...
unsigned char* ScanHeader(unsigned char pData)
{
    ...
    case S_PAYLOAD:                       // Find Header Flag, Then get length info
    {
        if(index == 4)
        {
            state = S_CRC;
    ...
    case S_CRC:                          // Check Header via CRC8
    {
        state     = S_IDLE;
        buffer[index] = pData;
        if(Verify_CRC8_Check_Sum(buffer, 6))
        {
            index = 0;
...
unsigned char FrameUnpack(unsigned char token, unsigned char* pBuffer)
{
    ...
    static unsigned char state = S_IDLE;
    static unsigned char __RecvFrameBuffer[PROTOCAL_FRAME_MAX_SIZE];
    ...
    case S_IDLE:                                    // Wait Header
    {
        ...
        if(( unsigned char*)0 != pHeader)
        {
            ...
            __RecvFrameBuffer[cnt++] = pHeader[4];
            __RecvFrameBuffer[cnt++] = pHeader[5];
    ...
    case S_PAYLOAD:                               // Receive Payload data
    {
        ...
        if(cnt == sizeof(tHeader) + PayLoadLen + sizeof(tCheckSum))
        {
            state = S_IDLE;
            if(Verify_CRC16_Check_Sum(__RecvFrameBuffer, sizeof(tHeader) + PayLoadLen + sizeof(tCheckSum)))
            {
                // CRC Check OK
                memcpy(pBuffer, __RecvFrameBuffer, sizeof(tHeader) + PayLoadLen + sizeof(tCheckSum));
                return 1;
            }
            else
            {
...
{   
    //define the header flag
    pDataOut[0] = PROTOCAL_tHeader;
    //assign the data length
    pDataOut[1] = len;

    //append the crc8 of header at the pDataOut[sizeof(tHeader)-1]
    Append_CRC8_Check_Sum(pDataOut, sizeof(tHeader));
    memcpy(&pDataOut[sizeof(tHeader)], &pDataIn[sizeof(tHeader)], len);

    //appent the crc16 at the pDataOut[len + sizeof(tHeader) + sizeof(tCheckSum) - 2]
    //and pDataOut[len + sizeof(tHeader) + sizeof(tCheckSum) - 1]

    Append_CRC16_Check_Sum(pDataOut, len + sizeof(tHeader) + sizeof(tCheckSum));
    
    //return the length of the packet
    return sizeof(tHeader) + len +  sizeof(tCheckSum);
}
